...
# Prebuild file before running test
node-gyp configure
node-gyp build
if [[ "$?" == 1 ]]; then
    echo "\033[31m\xE2\x9C\x96 Please check which addons build fail, abort git push\033[0m"
    exit 1
fi
echo "\033[32m\xE2\x9C\x94 Pass for addons build \033[0m"
